...
function gameLogic() {
    if (started == 1) {
        ...
        for (i = 0; i < enemy.length; i++) --enemy[i], enemy[i] >= 0 && (enemy[i] == playerPos ? (invincible == 1 ? (grid[enemy[i]] = "·" + timeLeft + "+", enemy[i] = 0, --score) : jump > 2 ? (playerJump(), enemy[i] = 0) : (grid[enemy[i]] = "·" + "×" + "!" + "×", enemy[i] = 0, --score, started = 0)) : (grid[enemy[i]] = "X"));
    ...
    } else clearInterval(runGame), button !== null && (button.innerHTML == "Sound: on" && document.getElementById("faaah").play()), grid = [], food = [], fruit = [], enemy = [], powerup = [], jump = 0, foodSpawn = 0, fruitSpawn = randomIntFromInterval(150, 80), enemySpawn = 30, powerupSpawn = randomIntFromInterval(250, 60), localStorage.getItem("highScore") < score && localStorage.setItem("highScore", score), setTimeout(function () {
...
function startGame() {
    document.onkeydown = function (o) {
        ...
        countingDown = 1;
        score = 0;
        button !== null && (button.innerHTML == "Sound: on" && document.getElementById("countdown").play());
...
function toggleSound() {
    button.innerHTML == "Sound: on" ? (button.innerHTML = "Sound: off", localStorage.setItem("soundPref", button.innerHTML)) : (button.innerHTML = "Sound: on", localStorage.setItem("soundPref", button.innerHTML), document.getElementById("toggle-sound").play())
}
